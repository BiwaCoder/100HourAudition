if(GameObject.Find("MemoryActionVerification"))return "already started";new GameObject("MemoryActionVerification");
...
var s=d.Game.State;d.Game.TelemetryEnabled=false;
...
ScreenCapture.CaptureScreenshot("Docs/MemoryActions/"+locale+".png");yield return null;journal.Close();
}
...
notes.Add("SOURCE "+d.LastDialogueSource);notes.Add("GENERATED "+s.actions.First(a=>a.id=="ai_topic").line);
Check(s.actions.First(a=>a.id=="ai_topic").recallA=="memory-action-test","AI result retains selected memory association");
}finally{HundredHour.Localization.GameLanguage.Select(old,false);System.IO.File.WriteAllLines("Docs/MemoryActions/Verification.txt",notes);}
}
